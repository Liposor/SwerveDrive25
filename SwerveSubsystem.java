  ...
  
{
  ...
  public double giroscopio = 0;
  public double limelightg = 0;
  public double offsetgiro = 0;
  public double finalgiro = 0;

  public double calculolime = 0;
  ...
  public SwerveSubsystem() 
  {
    ...
    var alliance4 = DriverStation.getAlliance();
    if (alliance4.get() == DriverStation.Alliance.Blue)
    {
      eblue = -1;
    }else 
    {
    ...
    if (alliance2.get() == DriverStation.Alliance.Red) 
    {
      ...
      LimelightHelpers.setCameraPose_RobotSpace(DriveConstants.limelightBack,0.130,0.259,0.870,0,0,0);
    ...
    }else
    {
      ...
      blueAlliance = true;
      LimelightHelpers.setCameraPose_RobotSpace(DriveConstants.limelightFront,0.110,0.259,0.410,0,0,0);
      LimelightHelpers.setCameraPose_RobotSpace(DriveConstants.limelightBack,0.130,0.259,0.870,0,25,0);
    ...
    m_poseEstimator =
                new SwerveDrivePoseEstimator(
                    ...
                    RobotConfig config = null; // Inicialização com null
                    try{
                    ...
                    // Verificar se a configuração foi carregada com sucesso
                    if (config == null) {
                        throw new RuntimeException("Failed to load RobotConfig from GUI settings.");
                    }
                    ...
                    AutoBuilder.configure(
                            ...
                            new PPHolonomicDriveController( // PPHolonomicController is the built in path following controller for holonomic drive trains
                                    new PIDConstants(0.0665, 0.0, 0.0), // Translation PID constants
                                    new PIDConstants(2.85, 0.0, 0.0) // Rotation PID constants
                            ),
  ...
  public void periodic() 
  {
    ...
    if (exampleXbox.getStartButtonPressed()){
      zeroHeading();
    }


    NetworkTableInstance.getDefault().getTable("limelight-f").getEntry("limelightg").getDouble(0);
    ...
    SmartDashboard.putBoolean("True Blue", blueAlliance);
 ...
 */
                                  ...
                                  Supplier<Boolean> fieldOrientedFunction)
  {
    ...
    double xSpeed = xSpdFunction.get() * eblue;
    double ySpeed = ySpdFunction.get() * eblue;  
   ...
   * @return
   */
public double getGyroRotation2d() {
    // Leitura do giroscópio em graus
    double giroscopio = gyro.getRotation2d().getDegrees();
    
    // Leitura da rotação vinda da Limelight (botpose[5] é o ângulo em graus)
    double[] botpose = NetworkTableInstance.getDefault()
        .getTable("limelight-f")
        .getEntry("botpose")
        .getDoubleArray(new double[6]);

        double[] botpose2 = NetworkTableInstance.getDefault()
        .getTable("limelight-t")
        .getEntry("botpose")
        .getDoubleArray(new double[6]);

    

    double limelightg = botpose[5];
    double limelightg2 = botpose2[5];
    double limelighto = limelightg + limelightg2 % 2;

    if (limelightg<0){
      calculolime = 360 - limelighto;
    }

    // Cálculo do offset entre Limelight e Giroscópio
    double offsetgiro = calculolime - giroscopio;

    double correcao = offsetgiro % 2;
    ...
    // Retorna o valor final em graus
    return limelighto;
}
}